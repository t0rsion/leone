...
Three facts that should drive a consumer engine's design:
...
These should be first-class backends. They cost almost nothing and can have high acceptance.
...
**Oilbird.** Diagnosing failures position-by-position across ten benchmarks, it finds that on dense tool-calling traffic *about half* of what the strongest exact-match drafter misses is already present in the pool but unreachable by exact lexical matching. Re-keying the same pool by the hidden state the verifier already computed at each committed token (no extra model) lifts accepted length 24-29% across three published drafters. **4.4× on API-Bank vs 3.9× for the best training-free baseline and 2.0× for EAGLE-3.** For an agentic workload this is a large gain for little new machinery.
...
Speculation monetizes *unused arithmetic intensity*. Continuous batching consumes that same headroom. Rough regimes: batch 1-4 is where it usually pays; batch 8-16 is often marginal for 7B-class dense targets; large models stay bandwidth-bound longer; long contexts become KV-bound earlier. The scheduler rule: shorten or disable when
