...
## 0. TL;DR for an engine designer

Five structural facts dominate everything below.

1. **The workload has shifted from dense to sparse MoE.** Almost every model people care about in 2026 — gpt-oss-120b (117B/5.1B), Qwen3.6-35B-A3B, Qwen3-Coder-Next-80B-A3B, GLM-5.x (744B/40B), Kimi K2.x (1T/32B), DeepSeek V4-Flash (284B/13B) and V4-Pro (1.6T/49B), MiniMax M2/M3, Gemma 4 26B-A4B — is MoE with a small active fraction. Token generation speed tracks **active parameters × memory bandwidth of wherever the experts live**, not total parameters.
2. **Therefore the canonical consumer setup is heterogeneous**: attention + shared experts + embeddings on GPU, routed expert FFNs in system RAM, executed on CPU. `--n-cpu-moe` / `-ot "exps=CPU"` is the single most important trick of the era.
3. **Prefill is the pain point, not decode.** Unified-memory boxes (Strix Halo, DGX Spark, Macs) and CPU-offload rigs generate tokens acceptably but time-to-first-token on a 32k–256k prompt is brutal. Agentic/coding workloads amplify this because they re-prefill constantly. Prompt cache reuse and cheap incremental prefill are worth more than 10% decode wins.
...
### 1.1 Capacity / bandwidth landscape
...
| Prosumer GPU | RTX PRO 6000 Blackwell | 96 GB GDDR7 | ~1792 GB/s | ~$8000+ | The "one card runs everything ≤120B" answer |
...
| Apple | M4 Pro / M4 Max / M5 Max / M5 Ultra | 24–192 GB | 273 / 546 / ~600 / ~1000+ GB/s | $2000–8000 | Mac Studio M3 Ultra 512 GB @ 819 GB/s is the big-model king |
...
- **Generation scales with active params**: ~3B active → 61–86 t/s; 5.1B active → ~53 t/s; ~10B active → 29–33 t/s; 22B active → ~17 t/s. Clean bandwidth-bound behaviour.
...
|---|---|---|---|---|
| 2× EPYC 9554 (128c), 1.5 TB DDR5-5600, 1× RTX 6000 Ada 48 GB, 126 threads | DeepSeek-R1 Q4_K_M | KTransformers v0.3 | ~150 t/s | ~11 t/s |
...
The typical decision tree people describe: *single user, weird hardware, newest model* → llama.cpp. *Single user, all-NVIDIA, want the best quality at a given VRAM* → ExLlamaV3/TabbyAPI. *Many users or agents* → vLLM/SGLang. *DeepSeek-class model with 512 GB RAM* → KTransformers or ik_llama.cpp. *Mac* → MLX or llama.cpp. *Don't want to think* → Ollama/LM Studio.
...
llama-server -m model.gguf \
  -ngl 999 \                       # everything to GPU first
...
- llama.cpp server slot save/restore + `--ctx-checkpoints N` (default 32; the Qwen3-Next-on-Mac writeup recommends **128** for multi-session robustness).
...
## 4. The model landscape people are actually running (mid-2026)
...
| GLM-5 / 5.1 / 5.2 | ~744B / 40B | MIT | 128 GB+ / Mac Studio 512 GB / EPYC. 1M ctx. GLM-5.2 ranked #1 open-weights on AA Intelligence Index v4.1 |
...
3. **Persistent, robust prompt caching.** The 84 s → 1.0 s number is the biggest single measured win in this entire survey and it is currently fragile and model-dependent.
...
9. **Batching that helps single users too.** The DGX Spark data (33.5 → 862 t/s from c=1 to c=256) shows how much is left on the table. Agentic workloads issue many parallel small requests; an engine that continuously batches *and* offloads MoE experts would occupy a genuinely empty niche between llama.cpp and vLLM.
