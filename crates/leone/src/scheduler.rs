...
impl ContinuousScheduler {
    ...
    pub fn complete_quantum(
        ...
        state.virtual_runtime = state
            ...
            .ok_or(SchedulerError::AccountingOverflow)?;
        self.stats.emitted_tokens = self
            .stats
            .emitted_tokens
            .checked_add(u64::from(emitted_tokens))
            .ok_or(SchedulerError::AccountingOverflow)?;
        if state.cancellation_requested {
            ...
            // Cancel is applied at the quantum boundary, so overshoot cannot
            // exceed this quantum.
            self.stats.max_cancellation_overshoot_tokens = self
                .stats
                .max_cancellation_overshoot_tokens
                .max(emitted_tokens);
    ...
    fn admission_rejection(
        ...
        if spec.priority == 0 || spec.priority > 100 {
            return Ok(Some(AdmissionReject::InvalidPriority));
        }
        if spec.deadline_ns.is_some_and(|deadline| deadline <= now_ns) {
            return Ok(Some(AdmissionReject::ExpiredDeadline));
        }
        let active = self
            .requests
            .values()
...
pub fn run_service_trace_json(bytes: &[u8]) -> Result<ServiceTraceReport, SchedulerError> {
    ...
    while next < ordered.len() || scheduler.has_runnable_requests() {
        ...
        if let Some(dispatch) = scheduler.dispatch(now)? {
            ...
            let source = &sources[&dispatch.request_id];
            let start = usize::try_from(
                scheduler
                    .emitted_tokens(dispatch.request_id)
                    .ok_or(SchedulerError::UnknownRequest(dispatch.request_id))?,
            )
            .map_err(|_| SchedulerError::AccountingOverflow)?;
            let count = usize::try_from(dispatch.token_budget)
                .map_err(|_| SchedulerError::AccountingOverflow)?
                .min(source.isolated_tokens.len().saturating_sub(start));
            let end_ns = now
                .checked_add(
                    u64::try_from(count)
                        .map_err(|_| SchedulerError::AccountingOverflow)?
...
pub fn verify_service_trace(
    ...
    let mut running: Option<(RequestId, u32, bool)> = None;
    let mut emitted: BTreeMap<RequestId, u64> = BTreeMap::new();
    for event in &report.events {
        match *event {
            ServiceEvent::Admission {
                ...
                if reservations.insert(request_id, reserved_kv_bytes).is_some() {
                    return Err(SchedulerError::InvalidTrace("duplicate admission".into()));
                }
                reserved = reserved
                    .checked_add(reserved_kv_bytes)
                    .ok_or(SchedulerError::AccountingOverflow)?;
            }
            ServiceEvent::Admission { .. } => {}
            ServiceEvent::Dispatch {
                ...
                if running.is_some()
                    || !reservations.contains_key(&request_id)
                    || token_budget == 0
                    || token_budget > trace.policy.service_quantum_tokens
                {
                    return Err(SchedulerError::InvalidTrace("invalid dispatch".into()));
                }
                running = Some((request_id, token_budget, false));
            }
            ServiceEvent::Cancel {
                request_id,
                running: true,
                ...
                let (active, budget, _) = running.ok_or_else(|| {
                ...
                running = Some((active, budget, true));
            }
            ServiceEvent::Cancel {
                request_id,
                running: false,
                ...
                if let Some(bytes) = reservations.remove(&request_id) {
                    reserved -= bytes;
                }
            }
            ServiceEvent::Complete {
                ...
                let (active, budget, cancelled) = running.take().ok_or_else(|| {
                    SchedulerError::InvalidTrace("completion without dispatch".into())
                })?;
                if active != request_id
                ...
                *emitted.entry(request_id).or_default() += u64::from(emitted_tokens);
                if status.is_terminal() {
                    if let Some(bytes) = reservations.remove(&request_id) {
                        reserved -= bytes;
                    }
                }
                if reserved != reserved_kv_bytes {
                    return Err(SchedulerError::InvalidTrace(
        ...
        if reserved > trace.policy.max_reserved_kv_bytes {
            return Err(SchedulerError::InvalidTrace("KV capacity exceeded".into()));
        }
    }
    if running.is_some() || reserved != 0 {
        return Err(SchedulerError::InvalidTrace(
    ...
    for result in &report.results {
        ...
        if result.status == RequestStatus::Finished
            && result.output_tokens != source.isolated_tokens
        {
            return Err(SchedulerError::InvalidTrace(
